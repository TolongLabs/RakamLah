...
test('rejects an out-of-order, duplicate, or unknown beat', () => {
  ...
  assert.throws(
    () => recorder.mark('result'),
    (error) => error.exitCode === 4 && /Expected beat start/.test(error.message)
  )
  recorder.mark('start')
  assert.throws(
    () => recorder.mark('start'),
    (error) => error.exitCode === 4 && /Expected beat result/.test(error.message)
  )
  assert.throws(
    () => recorder.mark('unknown'),
    (error) => error.exitCode === 4 && /Expected beat result/.test(error.message)
  )
})
...
test('audit rejects a capture that did not reach every beat', () => {
  ...
  assert.throws(
    () => recorder.audit(),
    (error) => error.exitCode === 4 && /Missing beats: result/.test(error.message)
  )
})

test('rejects an invalid expected beat contract before recording', () => {
  assert.throws(
    () => createBeatRecorder([], () => 0),
    (error) => error.exitCode === 2
  )
  assert.throws(
    () => createBeatRecorder(['start', 'start'], () => 0),
    (error) => error.exitCode === 2
  )
})
