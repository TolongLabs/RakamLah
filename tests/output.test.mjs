...
test('JSON mode emits exactly one parseable object to stdout', () => {
  ...
  const stderr = sink()
  writeResult(
    { ok: true, command: 'validate', artifacts: [] },
    { json: true, stdout: stdout.stream, stderr: stderr.stream }
  )
...
test('human mode prints the result message without serializing the envelope', () => {
  ...
  const stderr = sink()
  writeResult(
    { ok: true, command: 'doctor', message: 'All required tools are available.' },
    {
      json: false,
...
test('human mode directs failed result messages to stderr', () => {
  ...
  const stderr = sink()
  writeResult(
    { ok: false, command: 'verify', message: 'Audio stream is missing.' },
    {
      json: false,
      stdout: stdout.stream,
      stderr: stderr.stream
